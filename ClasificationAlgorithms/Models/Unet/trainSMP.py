...
def main(NUM_EPOCHS=NUM_EPOCHS):
    ...
    if LOAD_MODEL:
        load_checkpoint(torch.load("output_assets_model/best_model_checkpoint_UNet_smp.pth", weights_only=True), model)
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if SAVE_MODEL:
            ...
            if epoch_mean_dice >= best_dice:
                ...
                torch.save(checkpoint, "output_assets_model/best_model_checkpoint_UNet_smp.pth")
                with zipfile.ZipFile("output_assets_model/best_model_checkpoint_UNet_smp.zip", 'w') as zipf:
                    zipf.write("output_assets_model/best_model_checkpoint_UNet_smp.pth")
            else:
    ...
    if SAVE_MODEL:
        ...
        df_metrics.to_csv('output_assets_model/metrics_per_epoch_UNet_smp.csv', index=False)
        ...
        best_metrics_df.to_csv('output_assets_model/best_metrics_val(during_training)_UNet_smp.csv', index=True)
        ...
        pd.DataFrame([parameters]).to_csv('output_assets_model/parameters_UNet_smp.csv', index=False)
        with open('output_assets_model/parameters_UNet_smp.json', 'w') as json_file:
