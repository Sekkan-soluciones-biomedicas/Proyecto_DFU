...
checkpoint2 = torch.load("output_assets_model/best_model_checkpoint_UnetPP_smp.pth", weights_only=True)  ## Nota: el argumento weights_only=True es para evitar el warning que indica que de esta forma se carga con mayor seguridad el modelo. Sin embargo no se están cargando otros datos como el optimizador. En resumen, esto es solo para quitar el warning pues en principio no hay datos maliciosos en la forma en que se guarda el modelo localmente.
encoder_name = 'resnet34'  # Puedes cambiarlo a otro como 'efficientnet-b0' o 'resnet50'
...
df_test_metrics.to_csv("output_assets_model/test_metrics_UnetPP_smp.csv", index=True) # Sin índices.
...
df_test_mean_metrics.to_csv("output_assets_model/test_mean_metrics_UnetPP_smp.csv", index=True) # Sin índices.
