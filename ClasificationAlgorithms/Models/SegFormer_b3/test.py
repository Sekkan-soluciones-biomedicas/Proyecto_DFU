...
checkpoint2 = torch.load("output_assets_model/best_model_checkpoint_DeepLabV3+_smp.pth", weights_only=True)  ## Nota: el argumento weights_only=True es para evitar el warning que indica que de esta forma se carga con mayor seguridad el modelo. Sin embargo no se están cargando otros datos como el optimizador. En resumen, esto es solo para quitar el warning pues en principio no hay datos maliciosos en la forma en que se guarda el modelo localmente.
encoder_name = 'mit_b3'  # Puedes cambiarlo a otro como 'efficientnet-b0' o 'resnet50'
model2 = smp.Segformer(
        encoder_name=encoder_name,      # Ejemplo de backbone
        encoder_weights="imagenet",   # Pesos preentrenados
        in_channels=3,                # Canales de entrada (RGB)
        classes=4,                    # Número de clases (fondo, granulación, fibrinoso, calloso)
...
df_test_metrics.to_csv("output_assets_model/test_metrics_DeepLabV3+_smp.csv", index=True) # Sin índices.
...
df_test_mean_metrics.to_csv("output_assets_model/test_mean_metrics_DeepLabV3+_smp.csv", index=True) # Sin índices.
