...
import segmentation_models_pytorch as smp  # Nueva importación para usar smp.Unet
...
def main(NUM_EPOCHS=NUM_EPOCHS):
    # Definir el codificador para smp.Unet
    ...
    model = smp.MAnet(
        ...
        # Para usar PSCSE, descomentar la siguiente línea:
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'max', patience=15, verbose=True, factor=0.5, min_lr=1e-7)
    ...
    if LOAD_MODEL:
        load_checkpoint(torch.load("output_assets_model/best_model_checkpoint_MAnet_smp.pth", weights_only=True), model)
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if SAVE_MODEL:
            ...
            if epoch_mean_dice >= best_dice:
                ...
                torch.save(checkpoint, "output_assets_model/best_model_checkpoint_MAnet_smp.pth")
                with zipfile.ZipFile("output_assets_model/best_model_checkpoint_MAnet_smp.zip", 'w') as zipf:
                    zipf.write("output_assets_model/best_model_checkpoint_MAnet_smp.pth")
            else:
    ...
    if SAVE_MODEL:
        ...
        df_metrics.to_csv('output_assets_model/metrics_per_epoch_MAnet_smp.csv', index=False)
        ...
        best_metrics_df.to_csv('output_assets_model/best_metrics_val(during_training)_MAnet_smp.csv', index=True)
        ...
        pd.DataFrame([parameters]).to_csv('output_assets_model/parameters_MAnet_smp.csv', index=False)
        with open('output_assets_model/parameters_MAnet_smp.json', 'w') as json_file:
            json.dump(parameters, json_file, indent=4)

        print('Best model epoch:', best_model_epoch + 1)
