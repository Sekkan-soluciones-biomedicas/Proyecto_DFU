...
import torch
import pandas as pd
from main import AttentionUnet#, ResUnet, etc.
...
model1 = AttentionUnet(in_channels=3, out_channels=4).to(DEVICE)    ## ------------ Aquí al cambiar de modelo -------------.
...
model2 = AttentionUnet(in_channels=3, out_channels=4).to(DEVICE)    ## ------------ Aquí al cambiar de modelo -------------.
...
df_test_metrics.to_csv("output_assets_model/test_metrics_AttentionUnet.csv", index=True) # Sin índices.
...
df_test_mean_metrics.to_csv("output_assets_model/test_mean_metrics_AttentionUnet.csv", index=True) # Sin índices.
