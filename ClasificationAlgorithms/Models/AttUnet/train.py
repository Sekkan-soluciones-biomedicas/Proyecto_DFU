...
def main(NUM_EPOCHS=NUM_EPOCHS):
    ...
    model = AttentionUnet(in_channels=3, out_channels=4, dropout=p_dropout).to(DEVICE)
    ...
    if LOAD_MODEL:
        load_checkpoint(torch.load("C:/Users/am969/Documents/DFU_Proyect/ClasificationAlgorithms/Models/AttentionUnet/output_assets_model/best_model_checkpoint_AttentionUnet.pth",  weights_only=True), model)
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if SAVE_MODEL:
            ...
            if epoch_mean_dice >= best_dice:
                ...
                torch.save(checkpoint, "output_assets_model/best_model_checkpoint_AttentionUnet_semisup_ft.pth")
                # torch.save(checkpoint, "my_checkpoint.pth.tar")
                with zipfile.ZipFile("output_assets_model/best_model_checkpoint_AttentionUnet_semisup_ft.zip", 'w') as zipf:
                    zipf.write("output_assets_model/best_model_checkpoint_AttentionUnet_semisup_ft.pth")
            else:
    ...
    if SAVE_MODEL:
        ...
        df_metrics.to_csv('output_assets_model/metrics_per_epoch_AttentionUnet.csv', index=False)
        ...
        best_metrics_df.to_csv('output_assets_model/best_metrics_val(during_training)_AttentionUnet.csv', index=True)
